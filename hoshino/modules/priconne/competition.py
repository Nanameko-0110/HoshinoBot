...
sv = Service('pcr-competition', enable_on_default=True, bundle='pcr娱乐', help_='[@bot来局赛马]模拟来一局赛🐎')
...
@sv.on_fullmatch(('来局赛马'), only_to_me=True)
...
async def _select_(bot, ev):
    ...
    if uid != g_uid and g_result == []:
        await bot.finish(ev, f'上一场比赛已经结束，您可以用“@bot来局赛马”发起新的比赛', at_sender=True)
    if uid == g_uid:
