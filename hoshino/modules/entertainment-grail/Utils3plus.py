...
async def followerList(userQQ):
    ...
    p = os.getcwd() + '/hoshino/modules/entertainment-grail/Config/UserAssistance/Entourage.json'
...
async def extractBattleResultMessageSegment(winnerQQ, loserQQ):
    ...
    if winner == error or loser == error:
        raise GrailExcept
    p = os.getcwd() + '/hoshino/modules/entertainment-grail/Config/Battle/Plot.json'
    content = await Utils.readFileToJSON(p)
    if content == error:
        raise GrailExcept
...
async def judgeAttackCd(userQQ):
    ...
    p = os.getcwd() + '/hoshino/modules/entertainment-grail/User/Battle/CombatRecord_' + str(nowTimeAbbreviation) + '.json'
    content = await Utils.readFileToJSON(p)
...
async def addSingleStepCombatToken(userQQ, targetQQ, goodsList):
    p = os.getcwd() + '/hoshino/modules/entertainment-grail/User/Battle/Battle.json'
    content = await Utils.readFileToJSON(p)
    if content == error:
        battleFileStructure = {
            "battlelist": [],
