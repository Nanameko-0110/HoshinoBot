...
async def getOrWriteNumber(sequence, group, model = 'user'):
    ...
    # user
    if sequence != 0:
        p = os.getcwd() + '/hoshino/modules/entertainment-grail/User/SequenceCode/UserUID.json'
    ...
    # group
    if group != 0:
        p = os.getcwd() + '/hoshino/modules/entertainment-grail/User/SequenceCode/GroupUID.json'
        if not os.path.exists(p):
            firstGroup = {
                "grouplist": [
                        {
...
async def determineWhetherToRegister(bot, userQQ, userGroup):
    p = os.getcwd() + '/hoshino/modules/entertainment-grail/User/Data/' + str(userQQ) + '.json'
    pTemplate = os.getcwd() + '/hoshino/modules/entertainment-grail/Config/Register/Template.json'
    if not os.path.exists(p):
        # register
