...
async def singleAdventureExtract(bot, userQQ, userGroup):
    p = os.getcwd() + '/hoshino/modules/entertainment-grail/Config/Explore/SingleAdventure.json'
    content = await readFileToJSON(p)
    if content == error:
        raise GrailExcept
    replyMessage = ''
...
async def getMultipleExplorationEventTable(number):
    p = os.getcwd() + '/hoshino/modules/entertainment-grail/Config/Explore/MultipleExploration.json'
    if not os.path.exists(p):
        return error
