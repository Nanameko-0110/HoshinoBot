...
async def img(bot, ev, msg, uid):
    ...
    rnd_hex = '%08X' % random.randrange(16**8)
    image_path = R.img(f'image-generate/image/{uid}_{rnd_hex}.jpg').path
    ...
    if font_length[0]>5:
        ...
        img.save(image_path)
        await bot.send(ev, R.img(f'image-generate/image/{uid}_{rnd_hex}.jpg').cqcode, at_sender=True)
